...
public class Crud {
    ...
    public static Productos getProducto(int id) { //devuelve un objeto de clase Productos
        ...
        String sql = "SELECT p FROM Productos p WHERE p.id=";
...
//        producto.setCategoria("complementos");
    ...
    public static int actualizaProducto(Productos miProducto) {
        EntityManagerFactory factory = Persistence.createEntityManagerFactory("unidad");
        EntityManager manager = factory.createEntityManager();
        String sql = "UPDATE Productos p SET p.nombre = :nombre, p.imagen = :imagen, p.categoria = "
                + ":categoria, p.precio = :precio WHERE p.id = :id";
